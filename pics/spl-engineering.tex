...
\tikzstyle{label}=[draw=none, font={\footnotesize}] %% Configure size of edge labels here
\tikzstyle{deaeEdge}=[]
...
\renewcommand{\deaeText}[1]{\ifsolution\textcolor{blue}{#1}\fi}
\tikzstyle{deaeEdge}=[color=blue]
\tikzstyle{deaeCustomDevBox}=[draw=blue]
\tikzstyle{deaeCustomDevEdge}=[color=blue]
...
\begin{tikzpicture}
  ...
  \node[draw=none, anchor=north west] (delabel) at (de.north west) {\deaeText{Domain Engineering}};
  ...
  \node[draw=none, anchor=north west] (aelabel) at (ae.north west) {\deaeText{Application Engineering}};
  ...
  %%% Highlighting problem and solution space
  \ifsolution
    \node[draw = none, yshift=+3cm] (spacesTop) at ($(da)!0.5!(dd)$) {};
    \node[draw = none, yshift=-3cm] (spacesBot) at ($(aa)!0.5!(ad)$) {};
    \node[draw = none, left  = of spacesTop, anchor = north east] {\deaeText{problem space}};
    \node[draw = none, right = of spacesTop, anchor = north west] {\deaeText{solution space}};
    \draw[blue] (spacesTop) -- (spacesBot);
  \fi
\end{tikzpicture}
