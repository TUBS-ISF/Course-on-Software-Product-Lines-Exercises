...
{Implementierung einer Software-Produktlinie}{
	...
	\end{itemize}
	% todo: explicitly state that students should write a README file, share their feature model in some form, and that they are not allowed to make trivial stub implementations (their program should do something actually useful for someone).
	% include README
	% include feature model
	% do not implement stubs, implement a real product line with real use
	% include a variability mechanism that does not require users to edit the source code itself (pass configuration via file or command line)
}